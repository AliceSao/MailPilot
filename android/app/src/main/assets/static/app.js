...
(function () {
    ...
    // 内存缓存 — 避免反复 JSON.parse(localStorage) 的 I/O 开销
    var _dataCache = null;
    function _readData() {
        if (_dataCache === null) {
            try { _dataCache = JSON.parse(localStorage.getItem('emailData')); } catch(e) { _dataCache = null; }
    ...
    function _saveData(arr, doSync) {
        _dataCache = arr;
        try { localStorage.setItem('emailData', JSON.stringify(arr)); } catch(e) {}
...
_saveData(newData, true);
    ...
    window.changeItemsPerPage = function (value) {
        ...
        currentPage = 1;
        var data = filteredData || _readData();
        renderTable(data);
        renderPagination(data.length);
    };
    ...
    function processImport(content, delimiter) {
        ...
        lines.forEach(function (line) {
            var fields;
            if (isCSV) {
                // CSV 格式：email,password,clientId,refreshToken,group,tokenStatus,permissionType,tokenRenewedAt
                fields = line.split(',');
    ...
    window.exportBackup = function () {
        batchActionPrompt(t('exportBtn'), getSelectedEmails(), function(emails) {
            if (emails.length === (_readData()).length) {
    ...
    window.filterByStatusSelect = function () {
        ...
        if (status) {
            if (status === 'unchecked') {
                filteredData = data.filter(function (item) { return item.tokenStatus !== 'valid' && item.tokenStatus !== 'invalid' && item.tokenStatus !== 'checking'; });
            } else {
    ...
    function loadFromBackend() {
        fetch('/api/accounts').then(function (r) { return r.json(); }).then(function (serverData) {
            ...
            if (local.length === 0) {
                ...
                // Merge: update refreshToken from server, restore missing entries from server
                var serverMap = {};
                ...
                // Restore entries that are in server but missing from local
                var localEmails = {};
                local.forEach(function(l) { localEmails[l.email] = true; });
                serverData.forEach(function(s) {
                    if (!localEmails[s.email]) {
                        local.push(s);
                    }
                });
